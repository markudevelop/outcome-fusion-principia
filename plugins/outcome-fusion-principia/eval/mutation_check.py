...
MUTANTS: list[tuple[str, str, str, str]] = [
    ...
    ("tool results dropped from the transcript (judge sees calls, not outcomes)",
     "scripts/common.py",
     'elif kind == "tool_result":',
     'elif kind == "tool_result" and False:'),
    ("failed tool results stop being marked",
     "scripts/common.py",
     "{' [ERROR]' if item.get('is_error') else ''}",
     "{''}"),
    ("one huge tool result can swallow the transcript",
     "scripts/common.py",
     "    if len(text) <= per_block:",
     "    if True:"),
    # Head-only truncation would cut the pass/fail counts and tracebacks, which
    # land at the END of command output — exactly the part that decides a verdict.
    ("tool-result truncation drops the verdict-bearing tail",
     "scripts/common.py",
     "{text[-half:]}",
     "{''}"),
    # --- "Better framing": the feature must be broken-detectable, not just present
    ("compiler stops asking whether the request is the right one",
     "scripts/compile_prompt.py",
     "# Better framing\nThe user has explicitly asked to be challenged here",
     "# Better framing REMOVED\nThe user has explicitly asked to be challenged here"),
    ("system prompt stops licensing a challenge to the request",
     "scripts/compile_prompt.py",
     "Question the request itself, not only how to execute it.",
     "Execute the request as written."),
    ("framing arrives after scope is already locked",
     "scripts/compile_prompt.py",
     "This section is ADVISORY. It never changes scope",
     "This section is MANDATORY. It changes scope"),
    ("advisory framing becomes mandatory (blocks the mission)",
     "scripts/common.py",
     'MISSION_REQUIRED_SECTIONS = ("# mission", "# deliverables checklist")',
     'MISSION_REQUIRED_SECTIONS = ("# mission", "# deliverables checklist", "# better framing")'),
    ("offline fallback loses its framing placeholder",
     "scripts/common.py",
     "# Better framing\nNot compiled offline.",
     "# Framing omitted\nNot compiled offline."),
    ("gate may punish an unadopted framing",
     "scripts/release_gate.py",
     '10. The mission\'s "Better framing" section is ADVISORY.',
     "10. The mission's framing section is MANDATORY."),
